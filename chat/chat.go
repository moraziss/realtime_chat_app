...
func New(db *database.Conn) *Chat {
	...
	log.Println("chat: запуск event loop")
	go c.eventloop()
...
loop:
	for {
		select {
		case <-c.quit:
			log.Println("chat: event loop остановлен")
			break loop
		...
		case msg, ok := <-c.broadcast:
			...
			log.Printf("chat: тип=%s отправитель=%s получатель=%s\n",
			...
			case MessageTypeMessage:
				...
				if err != nil {
					log.Printf("chat: ошибка сохранения сообщения: %v\n", err)
					continue
				}
				msg.ID = msgID
			...
			case MessageTypeTask:
				...
				if err != nil {
					log.Printf("chat: ошибка обновления задачи: %v\n", err)
					continue
				}
			...
			case "read":
				...
				if err != nil {
					log.Printf("chat: ошибка MarkMessagesAsRead: %v\n", err)
				}
			...
			case "task_accept":
				...
				if err != nil {
					log.Printf("chat: ошибка AcceptTask: %v\n", err)
					continue
				}
				...
				if err != nil {
					log.Printf("chat: ошибка маршалинга метаданных: %v\n", err)
					continue
				}
...
func (c *Chat) Bind(uid UserID, sid SessionID) func() {
	log.Printf("chat: привязка пользователя=%s сессия=%s\n", uid, sid)
		...
		for {
			...
			if err := ws.ReadJSON(&msg); err != nil {
				if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway) {
					log.Printf("chat: неожиданное закрытие: %v\n", err)
				}
				break
			}
...
func (c *Chat) Join(uid UserID) {
	...
	if err != nil {
		log.Printf("chat: ошибка получения комнат: %v\n", err)
		return
	}
	for _, room := range rooms {
		...
		log.Printf("chat: пользователь %s вошёл в комнату %s\n", uid, room.RoomID)
	}
}
...
func (c *Chat) Leave(uid UserID) {
	...
	log.Printf("chat: пользователь %s вышел из всех комнат\n", uid)
}
...
func (c *Chat) Clear(sess *Session) {
	...
	log.Printf("chat: сессия %s очищена\n", sessionID)
}
