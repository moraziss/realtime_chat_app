...
func (c *Conn) GetRooms(userID string) ([]entity.UserRoom, error) {
	// ИСПРАВЛЕНИЕ: Добавили поддержку "чата с самим собой" (Self-chat)
...
func (c *Conn) CreateTaskWithMessage(task *entity.Task) (string, string, error) {
	tx, err := c.db.Begin()
	if err != nil {
		return "", "", err
	}
