...
func (c *Conn) GetExtendedStats(userID string) (map[string]interface{}, error) {
	...
	// 3. Наиболее часто встречающийся приоритет среди выполненных задач
	...
	// Условие владения проверяется прямо в ON, а не в WHERE после LEFT JOIN:
	// иначе день, в который менялись только чужие задачи, выпал бы из выборки
	// целиком вместо того, чтобы показать 0 выполненных задач пользователя.
	rows, err := c.db.Query(`
		SELECT
			d.day::date,
		...
		LEFT JOIN tasks t ON d.day = t.updated_at::date AND (
			t.created_by = $1
			OR t.assigned_to = $1
			OR (t.accepted_by @> jsonb_build_array($1::text))
		)
		GROUP BY d.day
