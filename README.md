...
| `ALLOWED_ORIGINS`| список разрешённых Origin через запятую — для WS и для CORS REST API (не задано — разрешён любой Origin) | `https://app.example.com` |
...
`rooms`, `user_rooms` (связь многие-ко-многим), `conversations` (сообщения; для
сообщений-носителей задачи `metadata` хранит только ссылку `task_id` — сами
поля задачи каждый раз подтягиваются свежими из `tasks`, а не дублируются),
`tasks` (включая `accepted_by` — кто из участников принял задачу),
`verification_codes`.

`init.sql` применяется официальным образом Postgres только при первом старте
контейнера (пустой volume). Если база уже существует, накатите файл вручную —
он написан идемпотентно (`CREATE TABLE IF NOT EXISTS`, `ADD COLUMN IF NOT
EXISTS`), поэтому безопасен для повторного запуска на уже работающей БД.
